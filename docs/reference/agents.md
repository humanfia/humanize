...
bounded unattended flow. Kimi takes `port`, `open_browser`, `log_level` and `web_title`, the
command line of the `kimi web` daemon its turns are submitted to — see
[the daemon Kimi is driven through](#the-daemon-kimi-is-driven-through). It is frozen,
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `read-only` | refused | `plan` mode | `read-only` sandbox | `--mode plan` | refused | only `read_file`, `grep`, `list_dir` | plan mode, which stops edits and not commands | without `bash`, `edit`, `write` | without `edit`, `write_file`, `run_shell_command` | `edit` and `bash` denied | `plan` mode |
...
| `bypass` | `--dangerously-skip-permissions` | `manual` mode, every request answered here | `danger-full-access` | `--force --sandbox disabled` | supported | `--yolo` | `auto` mode | — | `--approval-mode yolo` | — | `yolo` mode |
...
backend; `specs/agents.md` is the contract they have to keep.
