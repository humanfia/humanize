...
`load` accepts `ralph_loop`, `rlar`, `humanize1:gen-plan`, a path of your own —
...
```python
load("rlar")([builder, checker], task)     # arrives as Agents(actor=…, reviewer=…)
```
...
load("rlar")(agents, task, {"rounds": 9}, drives={"reviewer": careful})
```
...
async def run(agents: tuple[Agent, Agent], task: str) -> None:
    await asyncio.gather(
        load("rlar")([agents[0].clone()], task),
        load("rlar")([agents[1].clone()], task),
    )
```
...
[one.flow for one in running()] # ["planned", "humanize1:gen-plan"]
```
...
`chat ▸ rlar`. The [epic](/user/tracing#what-a-run-writes-down) records each call and
each return.
