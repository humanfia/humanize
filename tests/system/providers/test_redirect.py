...
The real thing: `hmz internal cred` spawned exactly as a turn spawns it, with a program
underneath that reads, writes, renames, lists a directory, forks, and dies of a signal, on
files that are all under `tmp_path`. Nothing here stands in for the supervisor, because the
supervisor is what there is to be wrong.

Every test below needs a machine that can trace -- a Linux whose kernel will let this process
ptrace a child of its own. Whether it can is answered by running one rather than by asking what
the kernel is -- a container without `CAP_SYS_PTRACE` has every module and can supervise
nothing -- and where it cannot, these say so and skip. Which is what makes them the system
tier: the one a gate is meant to be able to leave out.

The other half of the module is the table, which path is answered by which, and that is read
directly in `tests/unit/providers/test_redirect.py`. A dictionary and a command line need no
kernel at all, and a tier is a directory, so keeping them here would leave them out of the gate
along with the supervisor they do not need.

Whether this machine can trace, and how `hmz internal cred` is spawned, are
`tests/supervising.py`'s: several suites and a conftest ask, and a conftest cannot import a
test module.
"""
