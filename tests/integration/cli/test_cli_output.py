...
def test_a_run_written_for_a_program_cannot_have_a_stray_line_put_in_it(
    ...
    """A stray line is a stream that will not parse, so it lands on stderr instead.

    One line that is not JSON is the whole document unreadable to whatever was reading it,
    and a flow prints for a person as readily as humanize does.
    """
    with Out(as_json=True) as out:
        print("a flow said this")  # noqa: T201  -- a flow printing for a person, on purpose
...
def test_what_a_turn_answered_is_still_on_stdout_for_a_script(
    ...
    # Tokens alone for a model nobody lists, which is the ordinary case: `$0.00` beside a
    # turn that cost something would be a claim about a bill, and a wrong one.
    assert "$" not in said.err
...
def test_the_footer_says_what_the_turn_cost_in_money_where_the_model_is_listed(
    priced: str, capsys: pytest.CaptureFixture[str]
) -> None:
    """Nobody is watching a token count for its own sake, here any more than in the sheet."""
    agent = ShellAgent(AgentConfig(model=priced, effort="high"), name="builder")
    with Out() as out, Shown(out) as shown:
        shown.heard(
            agent,
            None,
