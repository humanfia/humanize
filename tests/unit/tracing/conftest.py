...
Codex, DSH, Kimi and ZCode logs of whoever ran the suite -- and still pass, off somebody
else's transcripts. ZCode is the one with no variable in that list: what moves its home is
`HOME` itself, so setting that is the whole of what keeps this off the developer's own.
"""
