...
Gathers the sessions asked for and returns the trace as a document.
`hmz.runtime.doing.epics.Epics.trace` and `.traced` are shells around it, and what the
interface offers on a run is reached through them.
