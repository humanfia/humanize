...
Everything humanize does to a flow: where flows come from, what one is called, running its file
to read it, what it says it drives, refusing one before it can cost anything, compiling an
atlas, walking the prophecy that came of it, and fetching the skills it named.

None of this MUST be a thing a flow names. What a flow is written against is `hmz.flows`,
specified in [flows.md](flows.md), and the whole reason this is a layer of its own is that a
module here can move, be renamed, be split or be rewritten without a single flow in anybody's
repository noticing. So everything here MAY name `hmz.flows`, and `hmz.flows` MUST NOT import
anything here: what a flow legitimately needs from this layer -- `load`, and the little that
goes with it -- MUST be handed through from there by name, fetched when a flow asks for it and
never at import.

Nothing here MUST drive a coding agent itself. That is `coganchor`, which this is written
against and which MUST name nothing here.

Everything this package offers MUST be fetched when it is named. A command line that only lists
the places flows come from MUST NOT pay for the `ast` of two readings, and neither MUST a menu
of flows.
...
The front door: every name the modules beside it offer, fetched out of the module it is written
in when it is asked for.

- It MUST answer with the same object that module holds, so that there is one of each however
  it was reached.
- A name this package does not offer MUST be refused the way any module refuses one, so that
  `from hmz.runtime.flowing import checking` goes on being the reading rather than this.
- Where one word would name two things, this MUST offer one of them under another word and
  MUST NOT rename it where it is written: a module has only one of it and needs no second
  name, and a package holding both needs the reader not to have to ask which is meant.
...
`load`, `running` and `container` are the three names here a flow itself writes, and MUST be
reachable as `hmz.flows.load` and the rest: a flow writes one import and that import is
`hmz.flows`. They MUST be handed through from there by name and MUST NOT be imported by it --
a flow that never calls another flow MUST NOT load any of this by writing `@flow`. Everything
else here MUST be humanize's alone.
...
- Copying a flow into the flows of your own MUST take the whole of it -- the entry point,
  whatever it imports beside it and the `skills/` it brings -- since that is what makes the
  copy a flow rather than half of one. It MUST refuse a name already taken there rather than
  write over it, and a copy that failed partway MUST leave nothing under the name: half a flow
  is a flow that will not run, cannot be copied again, and hides the one it was copied from.
...
What an atlas compiles to: the graph a run of one walks, and the file a flowverse may ship it
in. None of it MUST be a thing an atlas author writes -- what they write is the marks, in
`hmz.flows.atlas` -- which is why it is here and not beside them: a flow names what it
declares, and never what its declaration was turned into.
...
- Reading a shipped prophecy back MUST build nothing but a prophecy. A pickle names the class
  to build at every step, and this file is opened by a reading whose whole promise is that it
  executes nothing, so the classes it may name MUST be held to the tuples a prophecy is made
  of and bytes naming anything else MUST be read as bytes that are not a prophecy.
