...
What the reports become is the agent's own moments: one
:class:`~hmz.coganchor.agents.hooks.Occasion` at `PreToolUse` per thing observed, named `spawn`,
`read`, `write` or `connect` after what the runtime did rather than after any tool the CLI has -- so
a flow that wants only these hangs its hook with `tool="spawn"`, and one hung on a tool of the CLI's
own never sees them. They are told rather than asked: this layer is behind the call it is reporting,
so a hook that refuses one would be refusing something that has already happened, and nothing here
acts on a verdict. A flow that means to *stop* an agent doing something hangs its hook where the CLI
asks first.
...
if TYPE_CHECKING:
    ...
    from hmz.coganchor.agents.hooks import Hooks
...
#: :mod:`hmz.coganchor.backends`; this is where the layer that uses it spells its own.
...
def runtime() -> str:
    ...
    found = importlib.resources.files("hmz.coganchor.agents.preload").joinpath(RUNTIME)
...
class Watch:
    ...
    def __init__(self, hooks: Hooks) -> None:
        ...
        """
        # Weakly, for the reason `hmz.coganchor.agents.hooks.Gate` holds its own weakly: this object
        # is held strongly by the `weakref.finalize` that closes it, so holding the hooks back would
        # reach through them to whatever a flow's own hook captured -- the agent, most often -- and
        # an agent reachable from that registry is one that is never collected, so the finalizer
        # never fires and the socket, the directory and the thread all stay.
    ...
    def tells(self, line: str) -> None:
        ...
        Args:
          line: The report, as the runtime wrote it. One that says nothing readable is
            ...
            than trusted.
        """
        from hmz.coganchor.agents.hooks import Moment, Occasion
